...
HELP_MSG = \
    ...
    2. Use the Python SDK:
        ...
        # Use the model
        results = model.train(res_data="coco128.yaml", epochs=3)  # train the model
    ...
    3. Use the command line interface (CLI):
        ...
        - Train a detection model for 10 epochs with an initial learning_rate of 0.01
            yolo detect train res_data=coco128.yaml model=yolov8n.pt epochs=10 lr0=0.01

        - Predict a YouTube video using a pretrained segmentation model at image size 320:
            yolo segment predict model=yolov8n-seg.pt source='https://youtu.be/Zgi9g1ksQHc' imgsz=320

        - Val a pretrained detection model at batch-size 1 and image size 640:
            yolo detect val model=yolov8n.pt res_data=coco128.yaml batch=1 imgsz=640
...
def yaml_save(file='res_data.yaml', data=None):
    """
    Save YAML res_data to a file.

    Args:
        file (str, optional): File name. Default is 'res_data.yaml'.
    ...
    # Dump res_data to file in YAML format
    with open(file, 'w') as f:
        yaml.safe_dump(data, f, sort_keys=False, allow_unicode=True)


def yaml_load(file='res_data.yaml', append_filename=False):
    """
    Load YAML res_data from a file.
    ...
    Returns:
        (dict): YAML res_data and file name.
    """
