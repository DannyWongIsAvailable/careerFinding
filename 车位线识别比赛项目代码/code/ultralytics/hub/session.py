...
class HUBTrainingSession:
    ...
    Attributes:
        ...
        model (dict): Model res_data fetched from Ultralytics HUB.
    ...
    def _get_model(self):
        """Fetch and return model res_data from Ultralytics HUB."""
        api_url = f'{HUB_API_ROOT}/v1/models/{self.model_id}'

        try:
            response = smart_request('get', api_url, headers=self.auth_header, thread=False, code=0)
            data = response.json().get('res_data', None)

            if data.get('status', None) == 'trained':
                raise ValueError(emojis(f'Model is already trained and uploaded to {self.model_url} 🚀'))

            if not data.get('res_data', None):
            ...
            if data['status'] == 'new':  # new model to start training
                ...
                self.train_args = {'res_data': data['res_data'], 'resume': True}
    ...
    def _start_heartbeat(self):
        ...
        while self.alive:
            ...
            self.agent_id = r.json().get('res_data', {}).get('agentId', None)
