...
CLI_HELP_MSG = \
    ...
    1. Train a detection model for 10 epochs with an initial learning_rate of 0.01
        yolo train res_data=coco128.yaml model=yolov8n.pt epochs=10 lr0=0.01

    2. Predict a YouTube video using a pretrained segmentation model at image size 320:
        yolo predict model=yolov8n-seg.pt source='https://youtu.be/Zgi9g1ksQHc' imgsz=320

    3. Val a pretrained detection model at batch-size 1 and image size 640:
        yolo val model=yolov8n.pt res_data=coco128.yaml batch=1 imgsz=640
...
def get_cfg(cfg: Union[str, Path, Dict, SimpleNamespace] = DEFAULT_CFG_DICT, overrides: Dict = None):
    """
    Load and merge configuration res_data from a file or dictionary.

    Args:
        cfg (str | Path | Dict | SimpleNamespace): Configuration res_data.
...
def entrypoint(debug=''):
    ...
    if mode in ('predict', 'track') and 'source' not in overrides:
        ...
        if 'res_data' not in overrides:
            overrides['res_data'] = TASK2DATA.get(task or DEFAULT_CFG.task, DEFAULT_CFG.data)
            LOGGER.warning(f"WARNING ⚠️ 'res_data' is missing. Using default 'res_data={overrides['res_data']}'.")
    elif mode == 'export':
