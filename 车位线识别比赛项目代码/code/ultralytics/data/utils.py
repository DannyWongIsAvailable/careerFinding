...
def check_det_dataset(dataset, autodownload=True):
    ...
    for k in 'train', 'val':
        if k not in data:
            raise SyntaxError(
                emojis(f"{dataset} '{k}:' key missing ❌.\n'train' and 'val' are required in all res_data YAMLs."))
    if 'names' not in data and 'nc' not in data:
        raise SyntaxError(emojis(f"{dataset} key missing ❌.\n either 'names' or 'nc' are required in all res_data YAMLs."))
...
def check_cls_dataset(dataset: str, split=''):
    ...
    if not data_dir.is_dir():
        ...
        if str(dataset) == 'imagenet':
            subprocess.run(f"bash {ROOT / 'res_data/scripts/get_imagenet.sh'}", shell=True, check=True)
        else:
    ...
    val_set = data_dir / 'val' if (data_dir / 'val').exists() else None  # res_data/test or res_data/val
    test_set = data_dir / 'test' if (data_dir / 'test').exists() else None  # res_data/val or res_data/test
...
class HUBDatasetStats():
    ...
    Args:
        path (str): Path to res_data.yaml or res_data.zip (with res_data.yaml inside res_data.zip). Default is 'coco128.yaml'.
        task (str): Dataset task. Options are 'detect', 'segment', 'pose', 'classify'. Default is 'detect'.
        autodownload (bool): Attempt to download dataset if not found locally. Default is False.

    Usage
        from ultralytics.res_data.utils import HUBDatasetStats
    ...
    def __init__(self, path='coco128.yaml', task='detect', autodownload=False):
        ...
        try:
            # res_data = yaml_load(check_yaml(yaml_path))  # res_data dict
            data = check_det_dataset(yaml_path, autodownload)  # res_data dict
            if zipped:
                data['path'] = data_dir
        except Exception as e:
    ...
    def _unzip(self, path):
        """Unzip res_data.zip."""
        if not str(path).endswith('.zip'):  # path is res_data.yaml
...
def compress_one_image(f, f_new=None, max_dim=1920, quality=50):
    ...
    Usage:
        from pathlib import Path
        from ultralytics.res_data.utils import compress_one_image
...
def delete_dsstore(path):
    ...
    Usage:
        from ultralytics.res_data.utils import delete_dsstore
...
def zip_directory(dir, use_zipfile_library=True):
    ...
    Usage:
        from ultralytics.res_data.utils import zip_directory
