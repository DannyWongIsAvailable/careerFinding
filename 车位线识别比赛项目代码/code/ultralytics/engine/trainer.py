...
Usage:
    $ yolo mode=train model=yolov8n.pt res_data=coco128.yaml imgsz=640 epochs=100 batch=16
"""
import math
import os
import subprocess
...
class BaseTrainer:
    ...
    Attributes:
        ...
        data (str): Path to res_data.
        trainset (torch.utils.res_data.Dataset): Training dataset.
        testset (torch.utils.res_data.Dataset): Testing dataset.
        ...
        try:
            if self.args.task == 'classify':
                ...
                if 'yaml_file' in self.data:
                    self.args.data = self.data['yaml_file']  # for validating 'yolo train res_data=url.zip' usage
    ...
    def get_dataset(data):
        """
        Get train, val path from res_data dict if it exists. Returns None if res_data format is not recognized.
        """
    ...
    def get_dataloader(self, dataset_path, batch_size=16, rank=0, mode='train'):
        """
        Returns dataloader derived from torch.res_data.Dataloader.
        """
    ...
    def on_plot(self, name, data=None):
        ...
        self.plots[name] = {'res_data': data, 'timestamp': time.time()}
    ...
    def check_resume(self):
        ...
        if resume:
            try:
                ...
                # Check that resume res_data YAML exists, otherwise strip to force re-download of dataset
                ckpt_args = attempt_load_weights(last).args
                if not Path(ckpt_args['res_data']).exists():
                    ckpt_args['res_data'] = self.args.data
