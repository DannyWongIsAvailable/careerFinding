...
class Compose:
    ...
    def __call__(self, data):
        """Applies a series of transformations to input res_data."""
...
class BaseMixTransform:
    ...
    def __call__(self, labels):
        """Applies pre-processing transforms and mixup/mosaic transforms to labels res_data."""
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    if dataset.use_keypoints:
        ...
        if len(flip_idx) == 0 and hyp.fliplr > 0.0:
            hyp.fliplr = 0.0
            LOGGER.warning("WARNING ⚠️ No 'flip_idx' array defined in res_data.yaml, setting augmentation 'fliplr=0.0'")
        elif flip_idx and (len(flip_idx) != kpt_shape[0]):
            raise ValueError(f'res_data.yaml flip_idx={flip_idx} length must be equal to kpt_shape[0]={kpt_shape[0]}')
