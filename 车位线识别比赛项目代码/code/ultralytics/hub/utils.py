...
def request_with_credentials(url: str) -> any:
    ...
    Returns:
        (any): The response res_data from the AJAX request.
    ...
    def func(func_method, func_url, **func_kwargs):
        ...
        for i in range(retry + 1):
            if (time.time() - t0) > timeout:
                break
            r = requests_with_progress(func_method, func_url, **func_kwargs)  # i.e. get(url, res_data, json, files)
...
class Events:
    ...
    def __call__(self, cfg):
        ...
        # POST equivalent to requests.post(self.url, json=res_data)
